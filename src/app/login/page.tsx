...
export default function LoginPage() {
  const { login, redirectPath, setRedirectPath, isLoggedIn, isLoading } =
    useAuth();
  ...
  if (isLoading || (isLoggedIn && !isSuccess)) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#FAF9F6] gap-4">
        <Loader2
          className="animate-spin text-[#7B2D0A]"
          size={40}
          strokeWidth={1}
        />
        <p className="font-serif italic text-stone-400 animate-pulse">
          Authenticating...
        </p>
      </div>
    );
              ...
              >
                <div className="mb-12">
                  ...
                  <p className="text-stone-500 text-sm font-light">
                    {step === "input"
                      ? "Access your account via mobile number."
                    ...
                    <div className="space-y-8">
                      <div className="group relative">
                        ...
                        <div className="flex items-center border-b-[1px] border-stone-200 group-focus-within:border-[#7B2D0A] transition-all py-3">
                          <span className="text-stone-400 text-xl font-light mr-4">
                            +91
                          </span>
                          <input
                            type="tel"
                            placeholder="00000 00000"
                            className="bg-transparent w-full text-2xl outline-none font-light"
                            value={phone}
                            onChange={(e) =>
                      ...
                      <button
                        disabled={loading}
                        className="w-full h-16 bg-[#1C1C1C] text-white rounded-2xl font-bold uppercase text-[11px] tracking-[0.4em] hover:bg-[#7B2D0A] transition-all flex items-center justify-center gap-3"
                      >
                        {loading ? (
                    ...
                    <div className="space-y-10">
                      ...
                      >
                        {[...Array(4)].map((_, i) => (
                          <div
                            key={i}
                            className={`w-16 h-20 border rounded-2xl flex items-center justify-center text-3xl font-serif transition-all duration-500 ${otp.length === i ? "border-[#7B2D0A] bg-white ring-4 ring-[#7B2D0A]/5 shadow-sm" : "border-stone-200 bg-stone-50 text-stone-300"}`}
                          >
                            {otp[i] || ""}
                            {otp.length === i && (
                              <motion.div
                                layoutId="cursor"
                                className="w-[1px] h-8 bg-[#7B2D0A] absolute animate-pulse"
                              />
                            )}
                          </div>
                        ))}
                      ...
                      <div className="flex flex-col items-center gap-6">
                        <button
                          ...
                          className={`text-[11px] font-bold tracking-[0.2em] uppercase transition-all ${canResend ? "text-black hover:text-[#7B2D0A]" : "text-stone-300 cursor-not-allowed"}`}
                        >
                        ...
                        <button
                          ...
                          className="text-[10px] text-stone-400 hover:text-black font-bold uppercase tracking-widest transition-all"
                        >
                          ← Change Number
                        </button>
                      </div>
