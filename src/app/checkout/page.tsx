...
export default function CheckoutPage() {
  const { cart, totalPrice, discount, appliedCoupon } = useCart();
  const { isLoggedIn, isLoading, userPhone } = useAuth();
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-8">
        ...
        <div className="mb-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.back()}
              className="p-2 -ml-2 hover:bg-stone-100 rounded-full transition-colors cursor-pointer"
            >
              <ChevronLeft size={20} />
            </button>
            <h1 className="text-2xl md:text-3xl font-serif text-stone-900 tracking-tight italic">
              Secure Checkout
            </h1>
          </div>
        ...
        >
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-stone-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                ...
                <InputField
                  label={isPincodeLoading ? "Fetching State..." : "State *"}
                  name="state"
                  value={formData.state}
                  onChange={handleInputChange}
                  readOnly={!isPincodeLoading && formData.state !== ""}
                  className={formData.state ? "bg-stone-50 font-bold" : ""}
                  required
                />
              </div>

              <div className="mt-12">
                <p className="text-[10px] font-black uppercase tracking-widest text-stone-400 mb-5">
                  Save Address As
                </p>
            ...
            <div className="bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-stone-100">
              ...
              <div className="space-y-4">
                ...
                <PaymentOption
                  ...
                  onClick={setPaymentMethod}
                  subtext="Pay on delivery via Cash or QR"
                />
              </div>
            </div>
          </div>
          ...
          <aside className="lg:col-span-5 lg:sticky lg:top-40">
            <div className="bg-[#1C1C1C] text-white rounded-[2.5rem] p-8 md:p-10 shadow-2xl overflow-hidden relative">
                      ...
                      {cart.map((item) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[9px] text-stone-500 mt-1 uppercase font-bold">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-500 block">
                      Total Amount
                    </span>
              ...
              <button
                type="submit"
                className="hidden md:flex w-full mt-10 py-5 bg-[#D4AF37] text-black rounded-2xl font-black uppercase text-[11px] tracking-[0.3em] items-center justify-center gap-2 hover:bg-white transition-all active:scale-95 shadow-xl shadow-black/20"
              >
            ...
            <div className="mt-8 flex flex-col gap-3 px-4">
              <div className="flex items-center gap-3 text-stone-400 text-[9px] uppercase tracking-widest font-black">
                <ShieldCheck size={14} className="text-[#D4AF37]" /> Verified
                Secure Gateway
              </div>
              <div className="flex items-center gap-3 text-stone-400 text-[9px] uppercase tracking-widest font-black">
                <Truck size={14} className="text-[#D4AF37]" /> Insured Express
                Delivery
              </div>
            </div>
          </aside>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[100] md:hidden bg-white border-t border-stone-100 p-5 shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
        <div className="flex items-center gap-5">
          ...
          <button
            form="checkout-form"
            type="submit"
            className="flex-[1.5] bg-black text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 active:scale-95 shadow-lg"
          >
...
function InputField({ label, ...props }: any) {
  return (
    <div className="relative group">
      <input
        {...props}
        className={`peer w-full bg-transparent border-b border-stone-200 py-3 outline-none focus:border-[#7B2D0A] transition-all text-sm font-medium placeholder-transparent ${props.className || ""}`}
        placeholder={label}
      />
      <label
        className="absolute left-0 -top-4 text-[10px] font-bold uppercase tracking-widest text-stone-400 transition-all 
        peer-placeholder-shown:text-sm peer-placeholder-shown:top-3 peer-placeholder-shown:font-normal
        peer-focus:-top-4 peer-focus:text-[10px] peer-focus:text-[#7B2D0A] peer-focus:font-bold pointer-events-none uppercase"
      >
        {label}
      </label>
    </div>
...
function TypeBadge({ active, onClick, icon, label }: any) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex items-center gap-2 px-5 py-2.5 rounded-xl border text-[10px] font-bold uppercase tracking-widest transition-all ${active ? "bg-[#1C1C1C] border-[#1C1C1C] text-white shadow-xl" : "border-stone-200 text-stone-400 hover:border-stone-400"}`}
    >
      {icon} {label}
    </button>
  );
...
function PaymentOption({ id, label, icon, active, onClick, subtext }: any) {
  return (
    <button
      type="button"
      onClick={() => onClick(id)}
      className={`w-full flex items-center justify-between p-5 rounded-2xl border-2 transition-all ${active ? "border-[#7B2D0A] bg-[#7B2D0A]/5" : "border-stone-100 hover:border-stone-200"}`}
    >
      <div className="flex items-center gap-4">
        <div
        ...
        <div className="text-left">
          <p
            className={`text-[12px] font-bold uppercase tracking-tight ${active ? "text-stone-900" : "text-stone-500"}`}
          >
            {label}
          </p>
          {subtext && (
            <p className="text-[9px] text-stone-400 italic font-medium mt-0.5">
              {subtext}
            </p>
          )}
        </div>
      ...
      <div
        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${active ? "border-[#7B2D0A]" : "border-stone-200"}`}
      >
...
function SummaryRow({ label, value, isDiscount }: any) {
  return (
    <div
      className={`flex justify-between text-[11px] font-bold uppercase tracking-widest ${isDiscount ? "text-green-400" : "text-stone-500"}`}
    >
      <span>{label}</span>
