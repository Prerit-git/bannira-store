    ...
    >
      <div className="max-w-5xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-6">
          ...
          >
            <div className="space-y-10 relative">
              ...
              <TrackingPoint
                ...
                time={formatDate(order.createdAt, 1)}
              ...
              <TrackingPoint
                title="In Transit"
                time={formatDate(order.createdAt, 2)}
                desc="Your package is on its way."
              />
              <TrackingPoint
                title="Delivered"
                time={formatDate(order.createdAt, 3)}
                desc="Arriving at your doorstep."
              />
            </div>
          </motion.div>
          ...
          <motion.div
            variants={itemAnim}
            className="bg-white rounded-[2.5rem] p-8 border border-stone-100 shadow-sm"
          >
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-stone-400 mb-8 border-b border-stone-50 pb-4">
              Items Summary
            </h3>
            <div className="space-y-6">
              {groupItems(order.items).map((item: any, i: number) => (
                <div key={i} className="flex gap-6 items-center">
                  <img
                    src={item.image}
                    className="w-16 h-22 object-cover rounded-2xl bg-stone-50 shadow-sm"
                  />
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-bold text-stone-900 mt-2 font-poppins">
        ...
        <aside className="lg:col-span-5 space-y-6 lg:pt-12">
          <motion.div
            variants={itemAnim}
            className="bg-[#1C1C1C] rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden"
          >
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-xs text-stone-500 uppercase tracking-widest font-bold">
                ...
                <span className="text-white">
                  ₹
                  {(
                    order.tax || Math.round((order.subtotal || 0) * 0.12)
                  ).toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between text-xs text-stone-500 uppercase tracking-widest font-bold">
          ...
          >
            ...
            <p className="text-[10px] font-bold text-stone-400 border-t border-stone-50 pt-4 uppercase tracking-widest">
              Phone: {addr?.phone}
            </p>
          </motion.div>
...
function TrackingPoint({
  ...
  return (
    <div
      className={`flex gap-6 relative z-10 transition-opacity duration-500 ${active ? "opacity-100" : "opacity-25"}`}
    >
      <div
      ...
      <div>
        ...
        <p className="text-[9px] font-bold text-[#7B2D0A] uppercase mt-0.5 tracking-tighter">
          {time}
        </p>
        <p className="text-[11px] text-stone-500 mt-2 font-medium leading-relaxed italic">
          {desc}
        </p>
      </div>
    </div>
